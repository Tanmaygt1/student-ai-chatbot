  ...
  switch (topic) {
    ...
    case 'departments':
      if (dept && data.department) {
        ...
        const list = Object.values(VIT_DATA.departments)
          .map(d => `• ${d.full_name} (${d.seats} seats)`)
          .join('\n');
        return `🎓 **Courses at VIT Pune (B.Tech Programs):**\n\n${list}\n\n🔗 [All Departments](https://www.vit.edu/academics-at-vit/)`;
      }
...
export async function POST(request) {
  try {
    ...
    if (pending.topic && !topic && dept) {
      topic = pending.topic;
      clearPendingContext(sessionId);
    } else if (pending.dept && dept === null) {
      dept = pending.dept;
    }

    // Store detected dept in context for follow-ups
